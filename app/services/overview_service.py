...
class OverviewService:
    ...
    async def get_overview(self, user: UserDocument) -> OverviewResponse:
        ...
        if self._messages is not None:
            standalone_query_counts_by_agent = await self._messages.count_user_messages_by_agent(
                user_id,
                since=thirty_days_ago,
            )
            standalone_message_count = await self._messages.count_messages_by_user(user_id)
