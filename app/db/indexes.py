...
async def create_indexes(db: AsyncIOMotorDatabase) -> None:
    ...
    await db["agents"].create_index([("user_id", 1), ("updated_at", -1)])
    ...
    await db["agents"].create_index([("user_id", 1), ("status", 1), ("is_active", 1), ("priority", 1), ("created_at", -1)])
    ...
    await db["chats"].create_index([("user_id", 1), ("agent_id", 1), ("messages.id", 1)])
    ...
    await db["messages"].create_index([("user_id", 1), ("agent_id", 1), ("created_at", -1)])
    await db["memory"].create_index([("user_id", 1), ("session_id", 1)], unique=True)
    await db["knowledge_extraction_jobs"].create_index([("user_id", 1), ("created_at", -1)])
    await db["knowledge_extraction_jobs"].create_index("updated_at", expireAfterSeconds=3600)
